...
namespace Net8_OAuthWithOpenId.Services
{
    ...
    {
        static string keyAlg = "66007d41-6924-49f2-ac0c-e63c4b1a1730";
        readonly ClientStore _clientStore = new ClientStore();
        readonly ICodeStoreService _codeStoreService;
        readonly IConfiguration _configuration;

        public AuthorizeResultService(ICodeStoreService codeStoreService, IConfiguration configuration)
        {
        ...
        public TokenResponse GenerateToken(IHttpContextAccessor httpContextAccessor)
        {
            ...
            if (clientCodeChecker.IsOpenId)
            {
                ...
                {
                    new Claim("sub", "8369498118"),
                    new Claim("given_name", "Sachin Shankar Jawale"),
                ...
                id_token = new JwtSecurityToken(_configuration.GetValue<string>("NCPDPJWT:issuer"), request.ClientId, claims, signingCredentials: credentials,
                    expires: DateTime.UtcNow.AddMinutes(_configuration.GetValue<int>("NCPDPJWT:expiration_time")));
            ...
            var access_token = new JwtSecurityToken(_configuration.GetValue<string>("NCPDPJWT:issuer"), request.ClientId, claims_at, signingCredentials: credentials_at,
                expires: DateTime.UtcNow.AddMinutes(_configuration.GetValue<int>("NCPDPJWT:expiration_time")));
